...
## 7. c5444b63 staging 与后端回归修复实际结果（2026-09-05）

- `c5444b63` 隔离镜像构建成功；新 staging MySQL 曾启动并健康。
- 合成空库已从 base 迁移到 `b1c2d3e4f5a6`；纯合成旧 schema 从 `a1b2c3d4e5f6` 升级到 head 成功。
- 初次目标 SHA 最小快照测试结果：后端 68 通过、4 失败；分析引擎 163 通过、2 个已登记保护失败。4 个后端失败中 3 个是测试夹具问题，1 个是事件优先 legacy 边界实现问题。
- 两文件最小修复后，同账户隔离临时目录中的定向测试 4/4 通过，后端完整测试 72/72 通过，均显式使用 `-p no:cacheprovider`；临时目录已清理。
- 分析引擎两个保护测试未修改、未计入本轮新回归；保护文件哈希保持基线。
- 前端纯逻辑测试 7/7 通过，Vite 生产构建通过。backend/frontend 未因测试门禁启动；本轮 c544 staging MySQL 已停止，runtime、镜像、网络、卷和合成数据保留供后续复核。
- 生产、旧 `6a774457` staging、OpenClaw、systemd、cron 和生产数据库未修改；当前仍不宣称生产可用。
- 下一步：以包含两文件修复的新提交 SHA 重建独立 staging，再执行迁移、全量测试和运行验收。
