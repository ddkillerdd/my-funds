...
## [7.3] - 2026-09-23 — 多平台持仓与确认份额

### Added
- 持仓页增加“按基金汇总＋各平台账户明细”，精确录入支持账户别名；加减仓支持平台确认份额、确认日期和确认净值。
- 同一基金不同平台分别记账、统一分析；超额卖出直接拒绝，旧成本未知时不再按零成本错误重算。

### Verified
- 后端完整测试 144 项通过，前端生产构建通过（Vite 6.4.1，2103 个模块）。
- 固定提交 `30e88380673deec82aa5dc7debe31de5714cd339` 已推送；生产 backend/frontend 已分别切换到 `my-funds-production-backend:30e8838-multiplatform` 和 `my-funds-production-frontend:30e8838-multiplatform`。
- 生产 `/health`、前端页面、`confirmed_shares` API 字段及“按基金汇总”页面标记均验收通过；四只现有支付宝持仓可按基金汇总并按平台账户分列。
- 没有新增数据库迁移、没有修改真实持仓或 OpenClaw；发布前 Compose 快照和离线归档保留在服务器供回滚核验。

## [7.2] - 2026-09-22 — 四基金决策纪律与人工执行闭环

### Added
- 为当前四只支付宝基金补充14:00辅助行情映射、市场时区说明和数据质量门禁；未配置基金不伪造盘中方向。
- 增加每日20元、每月200元、单日最多一只基金非零买入、冷却期和实际成交金额反馈接口。
- 报告建议明确区分“建议金额”和“用户实际成交金额”，系统只辅助决策，不自动下单或修改持仓。

### Verified
- 固定提交 `425faf2d65d7a0cb6fe8e61ab14b3761da72c420` 已推送；后端完整测试 141 项通过，前端生产构建通过（2103 个模块）。
- 生产 backend/frontend 已分别切换到 `my-funds-production-backend:425faf2-decision` 和 `my-funds-production-frontend:425faf2-decision`；2026-09-22 只读复核确认各一个运行实例，`/health` 与前端 GET 均通过。
- OpenClaw 原固定任务 ID 保持不变，调度已改为工作日14:00、`Asia/Shanghai`；部署过程未立即运行任务、未生成报告、未发送邮件。
- 本次发布未执行数据库迁移、未修改真实持仓，并保留权限 `0600` 的更新前快照与离线发布归档。

## [7.2-prep] - 2026-09-05 — 持仓正确性最终准入第一阶段收口
