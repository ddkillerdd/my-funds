...
| 项目 | 参考值（依部署环境而定） |
|------|------|
| 操作系统 | Linux (Rocky/Debian/Ubuntu 均可) |
| 内核 | 默认发行版内核 |
| CPU | ≥ 4 核（建议） |
| 内存 | ≥ 4 GB（建议） |
| 磁盘 | ≥ 20 GB（数据 + 日志） |
...
**现象**：config.py 中硬编码了原开发者的数据库地址/密码（部署环境相关配置不应写死在代码里）
...
## 24. 2026-09-05 持仓正确性最终准入第一阶段

- A-D 持仓正确性生产实现已完成代码级验收；四组本地提交已完成，本地最终准入不改变生产实现和策略参数。
- 前端纯逻辑测试 7/7 通过；变更范围内 22 个 Python 文件 AST 解析通过；`git diff --check` 通过。
- 前端生产构建使用 `E:\NOdeJs1111111\npm.cmd run build` 已在允许启动 esbuild 子进程的执行环境中通过：Vite 6.4.1 转换 2103 个模块、耗时 10.67 秒，仅有大于 500 kB chunk 警告；此前受限沙箱中的 `spawn EPERM` 不作为代码/发布阻塞。
